...
const SideBar: FC<SideBarProps> = ({selectedCategory, categories, OnCategoryPress}) => {
    ...
    const indicatorPostion = useSharedValue(0)
    const animatedValues = categories.map(() => useSharedValue(0))
    
    useEffect(() => {
        let targetIndex = -1
        const categoryHeight = 100; // Adjust this value to match the actual height of your category items
        ...
        if (targetIndex !== -1) {
            indicatorPostion.value = withTiming(targetIndex * categoryHeight, { duration: 500 });
            runOnJS(() => {
                scrollViewRef.current?.scrollTo({
                    y: targetIndex * categoryHeight,
                    animated: true,
                });
            })();
        }
    ...
    const indicatorStyle = useAnimatedStyle(() => ({
        transform: [{translateY:indicatorPostion.value}]
    }))
